/**
 * Holder dashboard endpoint tests (FR-HOLD-01/02/03).
 *
 ...
 * filters against a seeded row array, so scoping is asserted through real
 * query semantics rather than a mocked call.
 */
...
/** Applies .eq()/.order() against a seeded `certificates` row array. */
...
function makeApp({ rows = [], user = HOLDER } = {}) {
  const app = express();
  app.use(express.json());
  app.use(
    '/api',
    createHolderRouter({
      adminClient: fakeDb(rows),
