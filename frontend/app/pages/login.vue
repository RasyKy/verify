...
// Which portal sent the visitor here, used only for the heading. Allowlisted
// rather than trusting the raw query value, to avoid an open redirect via an
// arbitrary `?redirect=`.
...
const portalLabel = computed(
  () => PORTAL_COPY[requestedPath.value ?? ''] ?? 'Institution issuer portal',
)
...
async function onSubmit() {
  ...
  try {
    ...
    // Where they go is decided by their ROLE, not by the ?redirect= they
    // arrived with — otherwise a holder following an /admin link would be sent
    // to a portal the guard immediately bounces them out of.
    const profile = await fetchCurrentUser(true)
    if (!profile) {
      // Signed in with Supabase, but the backend will not honour it: no
      // profile row, or the account is deactivated.
    ...
    // Honour the requested page only when the role is actually allowed there.
    const target =
      requestedPath.value && requestedPath.value.startsWith(home)
        ? requestedPath.value
        : home
