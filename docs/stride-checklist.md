...
| T-03 | Issuer wallet private key stolen | E | Key in Render env vars only; `.env` gitignored; validated at boot, never logged | ◐ | `git log --all -- backend/.env` → **empty, confirmed never committed**. `PRIVATE_KEY` is in the logger's redaction list. **Key-rotation procedure still unwritten — required before mainnet** |
...
| **Rate limiting defeated behind Render's proxy** — with `trust proxy` unset every client shares one bucket; with `true`, a client can spoof `X-Forwarded-For` | `app.set('trust proxy', 1)` — the hop count | ◐ |
